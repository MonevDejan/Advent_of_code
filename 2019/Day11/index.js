...
const getPaintedArea = (puzzleString, input) => {
    ...
    while (isRunung) {
        let standingColor;
        if (areaToPaint[`${robotPosition.x},${robotPosition.y}`]) {
            standingColor = areaToPaint[`${robotPosition.x},${robotPosition.y}`].value;
        }
        else {
            areaToPaint[`${robotPosition.x},${robotPosition.y}`] = { value: 0, painted: false }
            standingColor = 0;
        }

        const paintColor = runIntCode.next([standingColor]);
        const direction = runIntCode.next();

        isRunung = !direction.done;

        if (isRunung) {
            if (paintColor.value !== areaToPaint[`${robotPosition.x},${robotPosition.y}`].value) {
                areaToPaint[`${robotPosition.x},${robotPosition.y}`].value = paintColor.value;
                areaToPaint[`${robotPosition.x},${robotPosition.y}`].painted = true;
            }

            direction.value === 0 ? facingDirection += 90 : facingDirection -= 90;
...
let mainObject2 = getPaintedArea(input, 1);

let line = Array.apply(null, Array(50));
const arrayToPrint = [Array.apply(null, Array(50)), Array.apply(null, Array(50)), Array.apply(null, Array(50)), Array.apply(null, Array(50)), Array.apply(null, Array(50)), Array.apply(null, Array(50))];
