...
const getMainContainer = (inputWire1) => {
    ...
    inputArrayWire1.forEach(x => {
        const distance = parseInt(x.substr(1));

        switch (x[0]) {
            case "R": {
                for (let i = 0; i < distance; i++) {
                    container[`${width + i},${height}`] = stepsWire1;
                    stepsWire1++;
                }
                width += distance;
                break;
            ...
            case "U": {
                for (let i = 0; i < distance; i++) {
                    container[`${width},${height + i}`] = stepsWire1;
                    stepsWire1++;
                }
                height += distance;
                break;
...
// if proerty already exist, push into corresponding array
const getArraysResult = (container, inputWire2) => {

    const inputArrayWire2 = inputWire2.split(",");
    ...
    inputArrayWire2.forEach(x => {
        let distance = parseInt(x.substr(1));
        switch (x[0]) {
            case "R": {
                for (let i = 0; i < distance; i++) {
                    if (container[`${width + i},${height}`]) {
                        distanceFromStart.push(Math.abs(width + i) + Math.abs(height));
                        numberOfSteps.push(container[`${width + i},${height}`] + stepsWire2);
                    }
                    stepsWire2++;
                }
                width += distance;
            ...
            case "U": {
                for (let i = 0; i < distance; i++) {
                    if (container[`${width},${height + i}`]) {
                        distanceFromStart.push(Math.abs(width) + Math.abs(height + i));
                        numberOfSteps.push(container[`${width},${height + i}`] + stepsWire2);
                    }
                    stepsWire2++;
                }
    ...
    return [distanceFromStart, numberOfSteps]
}

const mainObjectContainer = getMainContainer(wire1);
const result = getArraysResult(mainObjectContainer, wire2);

console.log(Math.min(...result[0]));
console.log(Math.min(...result[1]));
